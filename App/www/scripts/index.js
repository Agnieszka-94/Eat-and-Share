...
function addRestaurant(name, price, localization) {
    ...
    var id = findRestarantIdByName(name);

    addPrice(id, Number(price));
}

function findRestarantIdByName(name) {
    var database = firebase.database();
    var ref = database.ref('Lokale/');
    ref.on("child_added", function (snapshot) {
        if (snapshot.val().nazwa == name) {
            return snapshot.key;
        }
    });
...
function generateIndexOfRestaurants(reustaurantToShow) {
    ...
    $('.addTofavorites').on("click", handleAddToFavorites);
...
function initGoogleMapForShowMeTheWay(id, callback) {
    console.log('initGoogleMapForShowMeTheWay');
    var currentLocation = getCurrentLocationWithCallback(function (location) {
...
function getAllPrices(keyOfProduct) {
    var prices = [];
    var database = firebase.database();
    var ref = database.ref('Lokale/' + keyOfProduct + "/ceny");
    ref.on("child_added", function (snapshot) {
        if (snapshot.val().cena) {
            prices.push(snapshot.val().cena);
        }
    });
    return prices;
}

function averagePrice(keyOfProduct) {
    var prices = getAllPrices(keyOfProduct);
    if (prices && prices.length > 0) {
        var sumOfPrices = 0;
        for (var i = 0; i < prices.length; i++) {
            sumOfPrices += Number(prices[i]);
        }
...
function handleAddPrice() {
    ...
    addPrice(selectedRestaurant, priceToAdd);
    var average = averagePrice(selectedRestaurant);
    var averageRouded = average.toFixed(2);
    updatePrice(selectedRestaurant, average);
    clearPriceForm();
    getAllRestaurants($('#range-7a').val(), $('#range-7b').val());
}
...
function updatePrice(keyOfProduct, price) {
    var database = firebase.database();
    var ref = database.ref('Lokale/' + keyOfProduct);
...
function addPrice(keyOfProduct, price) {
    var database = firebase.database();
    var ref = database.ref('Lokale/' + keyOfProduct + "/ceny");
    if (price && !isNaN(price)) {
        var cena = { cena: price };
